...
function op_insert($kind,$sn=""){
    ...
    if($_FILES['pic']['name']) {
        ...
        if ($_FILES['pic']['error'] === UPLOAD_ERR_OK){
            ...
            #檢查資料目錄
            mk_dir(_WEB_PATH . "/uploads"); //確認有沒有uploads資料夾,沒有則幫忙建立
            mk_dir(_WEB_PATH . "/uploads" . $sub_dir); //確認uploads底下有沒有子目錄$sub_dir
            $path = _WEB_PATH . "/uploads" . $sub_dir . "/"; // 存檔位置
            #圖片名稱
            ...
            $file_name = $rand . "_" . $sn . "." . $ext ; // 檔案名稱格式
...
function getKindsBySn($sn){
    ...
    $row['pic'] = getFilesByKindColsnSort($kind,$sn); //global $kind來取得圖片
    return $row;
...
function op_form($kind,$sn=""){
    ...
    $row['pic'] = isset($row['pic']) ? $row['pic'] : "";  //圖片預設值
...
function op_list($kind){
    ...
    while($row = $result->fetch_assoc()){ 
        ...
        $row['pic'] = getFilesByKindColsnSort($kind,$row['sn']); //顯示圖片
        $rows[] = $row;
    }
