...
CREATE TABLE `kinds` (
    ...
    `ofsn` smallint(5) unsigned NOT NULL DEFAULT 0 COMMENT '父類別', -- 整個資料庫都要記錄在這裡,所以加這個類別分類
...
-- 上傳檔案資料表
CREATE TABLE `files` (
    `sn` smallint(5) unsigned NOT NULL AUTO_INCREMENT COMMENT 'files_sn', -- 檔案很多就加大這個值
    `kind` varchar(255) NOT NULL DEFAULT '' COMMENT '分類',
